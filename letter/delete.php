...
if ($message_id > 0) {
    ...
    if ($stmt) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            echo "<script>alert('삭제 실패: " . htmlspecialchars(mysqli_stmt_error($stmt)) . "'); history.back();</script>";
        }
        mysqli_stmt_close($stmt);
    } else {
        echo "<script>alert('쿼리 준비 실패: " . htmlspecialchars(mysqli_error($con)) . "'); history.back();</script>";
    }
} else {
